        ...
        <div class="bg-gradient-to-r from-yellow-500 to-yellow-600 rounded-lg p-6 text-white">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold">{{ $dueTodayCount }}</p>
                </div>
        ...
        <div class="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg p-6 text-white">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold">Rp {{ number_format($totalPendingAmount, 0, ',', '.') }}</p>
                </div>
        ...
        <div class="bg-gradient-to-r from-purple-500 to-purple-600 rounded-lg p-6 text-white">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold">{{ $debtReminders->total() }}</p>
                </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($debtReminders as $reminder)
                        <tr class="hover:bg-gray-50 {{ $reminder->status === 'pending' && $reminder->reminder_date && $reminder->reminder_date->isPast() ? 'bg-red-50' : '' }}">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div>
                                    <div class="text-sm font-medium text-gray-900">{{ $reminder->debtor_name }}</div>
                                    @if($reminder->notes)
                                        <div class="text-sm text-gray-500">{{ Str::limit($reminder->notes, 50) }}</div>
                                    @endif
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $reminder->description }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div>
                                    <div class="text-sm font-medium text-gray-900">{{ $reminder->reminder_date ? $reminder->reminder_date->format('d/m/Y') : '-' }}</div>
                                    @php
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $reminder->capitalTracking->name ?? 'N/A' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @php
                        ...
                        $statusText = 'Jatuh Tempo';
                                ...
                                @endphp
                                <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full {{ $badgeClass }}">{{ $statusText }}</span>
                                @if($reminder->paid_at)
                                    <div class="text-xs text-gray-500 mt-1">Dibayar: {{ $reminder->paid_at->format('d/m/Y') }}</div>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($reminder->contact_info)
                                    <div class="text-sm text-gray-500">{{ $reminder->contact_info }}</div>
                                @else
                                ...
                                <div class="flex space-x-2">
                                    @if($reminder->status === 'pending')
                                        <button type="button" 
                                                ...
                                                wire:click="markAsPaid({{ $reminder->id }})"
                                                title="Tandai Lunas">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                            </svg>
                                        </button>
                                    @elseif($reminder->status === 'paid')
                                        <button type="button" 
                                                ...
                                                wire:click="markAsPending({{ $reminder->id }})"
                                    ...
                                    <button type="button" 
                                            ...
                                            wire:click="confirmDelete({{ $reminder->id }})"
                                            title="Hapus">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="flex items-center justify-between mb-4">
                                <h3 class="text-lg font-medium text-gray-900" id="modal-title">
                                    {{ $editingId ? 'Edit Pengingat Hutang' : 'Tambah Pengingat Hutang' }}
                                </h3>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" 
                                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('debtor_name') border-red-500 @enderror" 
