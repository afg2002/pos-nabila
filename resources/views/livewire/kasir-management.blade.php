        ...
        <div class="bg-white rounded-2xl shadow-modern p-6 hover-glow transition-all duration-300 animate-slideInRight" style="animation-delay: 0.1s">
            <div class="flex items-center">
                ...
                <div class="ml-4 flex-1">
                    ...
                    <p class="text-2xl md:text-3xl font-bold text-gray-900">{{ number_format($totalTransactions) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-modern p-6 hover-glow transition-all duration-300 animate-slideInRight" style="animation-delay: 0.2s">
            <div class="flex items-center">
                ...
                <div class="ml-4 flex-1">
                    ...
                    <p class="text-2xl md:text-3xl font-bold text-gray-900">Rp {{ number_format($totalSales, 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse($sales as $sale)
                        <tr class="hover:bg-blue-50 transition-colors duration-150 @if($sale->status === 'CANCELLED') opacity-60 @endif">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div class="text-xs text-gray-500">
                                    {{ $sale->saleItems->count() }} item
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex px-3 py-1.5 text-xs font-bold rounded-full
                                    @if($sale->payment_method === 'cash') bg-green-100 text-green-800 border border-green-200
                                    @elseif($sale->payment_method === 'transfer') bg-blue-100 text-blue-800 border border-blue-200
                                    @elseif($sale->payment_method === 'edc') bg-purple-100 text-purple-800 border border-purple-200
                                    @elseif($sale->payment_method === 'qr') bg-orange-100 text-orange-800 border border-orange-200
                                ...
                                <div class="flex items-center justify-center space-x-1">
                                    ...
                                    <div class="relative" x-data="{ open: false }">
                                        ...
                                        <div x-show="open"
                                            ...
                                            <button wire:click="printReceiptThermal({{ $sale->id }}); open = false"
                                            ...
                                            <button wire:click="exportReceiptPNG({{ $sale->id }}); open = false"
                                            ...
                                            <button wire:click="exportReceiptPDFThermal({{ $sale->id }}); open = false"
                                            ...
                                            <button wire:click="exportInvoiceA4({{ $sale->id }}); open = false"
                                    ...
                                    <button wire:click="confirmDelete({{ $sale->id }})"
                                            ...
                                            @if($sale->status === 'CANCELLED') disabled @endif>
                ...
                <div class="p-4 border-b border-gray-200 hover:bg-gray-50 transition-colors animate-slideInUp">
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-3">
                        <div class="text-center">
                            ...
                            <div class="text-xs text-gray-500">{{ $sale->saleItems->count() }} item</div>
                        </div>
                        <div class="text-center">
                    ...
                    <div class="flex gap-2">
                        ...
                        <div class="flex-1 relative" x-data="{ open: false }">
                            ...
                            <div x-show="open"
                                ...
                                <button wire:click="printReceiptThermal({{ $sale->id }})"
                                        @click="open = false"
                                        class="w-full px-3 py-2 text-left text-sm hover:bg-blue-50 flex items-center gap-2 first:rounded-t-lg transition-colors">
                                    <i class="fas fa-print text-blue-600 w-4"></i>
                                    <span>Thermal</span>
                                </button>
                                <button wire:click="exportReceiptPNG({{ $sale->id }})"
                                        @click="open = false"
                                        class="w-full px-3 py-2 text-left text-sm hover:bg-green-50 flex items-center gap-2 border-t transition-colors">
                                    <i class="fas fa-image text-green-600 w-4"></i>
                                    <span>PNG</span>
                                </button>
                                <button wire:click="exportReceiptPDFThermal({{ $sale->id }})"
                                        @click="open = false"
                                        class="w-full px-3 py-2 text-left text-sm hover:bg-purple-50 flex items-center gap-2 border-t transition-colors">
                                    <i class="fas fa-file-pdf text-purple-600 w-4"></i>
                                    <span>PDF</span>
                                </button>
                                <button wire:click="exportInvoiceA4({{ $sale->id }})"
                                        @click="open = false"
                                        class="w-full px-3 py-2 text-left text-sm hover:bg-orange-50 flex items-center gap-2 border-t last:rounded-b-lg transition-colors">
                                    <i class="fas fa-file-invoice text-orange-600 w-4"></i>
                                    <span>Invoice</span>
                ...
                <div class="mt-6">
                    ...
                    <div class="bg-gray-50 rounded-lg p-4 mb-3">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
                            <button wire:click="printReceiptThermal({{ $selectedSale->id }})"
                            ...
                            <button wire:click="exportReceiptPNG({{ $selectedSale->id }})"
                            ...
                            <button wire:click="exportReceiptPDFThermal({{ $selectedSale->id }})"
                            ...
                            <button wire:click="exportInvoiceA4({{ $selectedSale->id }})"
                    ...
                    <div class="flex flex-wrap justify-end gap-2">
                        ...
                        <button wire:click="confirmDelete({{ $selectedSale->id }})"
                                class="px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-md hover:bg-red-700"
                                @if($selectedSale->status === 'CANCELLED') disabled @endif>
                            <i class="fas fa-trash mr-2"></i>
                            Hapus
                        </button>
                    </div>
                ...
                @if($confirmDeleteId)
                    ...
                    @if($saleToDelete)
                        <div class="bg-gray-50 rounded-lg p-3 mb-4">
                            ...
                            <div class="font-medium text-gray-900">{{ $saleToDelete->sale_number }}</div>
                            <div class="text-sm text-gray-600">Total: Rp {{ number_format($saleToDelete->final_total, 0, ',', '.') }}</div>
                        </div>
                    @endif
                @endif
        ...
        <div id="print-receipt-area" class="hidden">
            <div class="receipt-paper">
                ...
                <div class="text-center mb-3" style="line-height: 1.4;">
                    <h2 class="text-lg font-bold mb-1">{{ config('app.name', 'TOKO') }}</h2>
                    <p class="text-sm mb-1">Struk Pembayaran</p>
                    <p class="text-xs">{{ $selectedSale->created_at->format('d/m/Y H:i:s') }}</p>
                </div>
                ...
                <div class="border-t border-b border-dashed py-2 mb-2">
                    <table class="w-full text-sm" style="line-height: 1.4;">
                        <tr>
                            ...
                            <td class="text-right pb-0.5">{{ $selectedSale->sale_number }}</td>
                        </tr>
                        <tr>
                            <td class="pb-0.5">Kasir</td>
                            <td class="text-right pb-0.5">{{ $selectedSale->cashier->name ?? 'System' }}</td>
                        </tr>
                        @if($selectedSale->customer_name)
                            <tr>
                                <td>Pelanggan</td>
                                <td class="text-right">{{ $selectedSale->customer_name }}</td>
                            </tr>
                        @endif
                    </table>
                </div>
                    ...
                    @foreach($selectedSale->saleItems as $item)
                        <div class="mb-1.5" style="page-break-inside: avoid;">
                            <div class="font-medium" style="margin-bottom: 2px;">
                                {{ optional($item->product)->name ?? ($item->custom_item_name ?? 'Item') }}
                            </div>
                ...
                <div class="border-t border-dashed pt-2 mt-2">
                    <table class="w-full text-sm" style="line-height: 1.4;">
                        <tr>
                            ...
                            <td class="text-right pb-0.5">Rp {{ number_format($selectedSale->subtotal, 0, ',', '.') }}</td>
                        </tr>
                        @if($selectedSale->discount_total > 0)
                            <tr>
                                <td class="pb-0.5">Diskon</td>
                                <td class="text-right text-red-600 pb-0.5">-Rp {{ number_format($selectedSale->discount_total, 0, ',', '.') }}</td>
                            </tr>
                        @endif
                        <tr class="font-bold text-base">
                            <td class="py-1">TOTAL</td>
                            <td class="text-right py-1">Rp {{ number_format($selectedSale->final_total, 0, ',', '.') }}</td>
                        </tr>
                        <tr>
                            <td class="pb-0.5">Bayar ({{ ucfirst($selectedSale->payment_method) }})</td>
                            <td class="text-right pb-0.5">Rp {{ number_format(($selectedSale->cash_amount ?? 0) + ($selectedSale->qr_amount ?? 0) + ($selectedSale->edc_amount ?? 0), 0, ',', '.') }}</td>
                        </tr>
                        <tr>
                            <td>Kembalian</td>
                            <td class="text-right">Rp {{ number_format($selectedSale->change_amount, 0, ',', '.') }}</td>
                        </tr>
                    </table>
                </div>
