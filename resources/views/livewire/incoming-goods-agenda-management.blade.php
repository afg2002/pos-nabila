            ...
            <div class="bg-green-600 p-6 text-white">
                <div class="flex justify-between items-center">
                    ...
                    <div class="flex items-center space-x-2 bg-white/10 rounded-lg p-1">
                        ...
                        <h3 class="text-lg font-semibold px-4 min-w-[140px] text-center text-white bg-transparent">{{ \Carbon\Carbon::parse($filterMonth . '-01')->format('F Y') }}</h3>
                        <button type="button" 
                                class="p-2 text-white hover:bg-white/20 rounded-md transition-all duration-200 transform hover:scale-110"
                ...
                <div class="grid grid-cols-7 gap-2">
                    @foreach($calendarData as $day)
                        <div class="group relative min-h-[120px] p-3 rounded-xl border-2 transition-all duration-300 cursor-pointer transform hover:scale-[1.02] hover:shadow-lg
                                    {{ $day['isCurrentMonth'] ? 'bg-white border-gray-200 hover:border-green-300' : 'bg-gray-100 border-gray-100 hover:border-gray-300' }} 
                                    {{ $day['isToday'] ? 'ring-2 ring-green-500 border-green-300 bg-green-50' : '' }}"
                             wire:click="selectDate('{{ $day['date'] }}')">
                            
                            <!-- Date Number with Indicator -->
                            <div class="flex items-center justify-between mb-2">
                                <div class="relative">
                                    <span class="text-lg font-bold {{ $day['isCurrentMonth'] ? ($day['isToday'] ? 'text-green-700' : 'text-gray-900') : 'text-gray-400' }}">
                                        {{ $day['day'] }}
                                    </span>
