<div>
    ...
    <div class="bg-white shadow rounded-lg overflow-hidden">
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($capitalTrackings as $capital)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div>
                                    <div class="text-sm font-medium text-gray-900">{{ $capital->name }}</div>
                                    @if($capital->description)
                                        <div class="text-sm text-gray-500">{{ $capital->description }}</div>
                                    @endif
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $capital->current_amount > 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                    Rp {{ number_format($capital->current_amount, 0, ',', '.') }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @php
                                ...
                                <div class="w-full bg-gray-200 rounded-full h-2.5">
                                    <div class="h-2.5 rounded-full {{ $percentage > 80 ? 'bg-red-600' : ($percentage > 50 ? 'bg-yellow-600' : 'bg-green-600') }}" 
                                         style="width: {{ max(0, min(100, $percentage)) }}%"></div>
                                </div>
                                <div class="text-xs text-gray-500 mt-1">{{ number_format($percentage, 1) }}%</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $capital->is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                    {{ $capital->is_active ? 'Aktif' : 'Nonaktif' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $capital->creator->name ?? 'System' }}
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex space-x-2">
                                    ...
                                    <button type="button" 
                                            wire:click="toggleStatus({{ $capital->id }})"
                                            class="{{ $capital->is_active ? 'text-yellow-600 hover:text-yellow-900' : 'text-green-600 hover:text-green-900' }} transition-colors duration-200"
                                            title="{{ $capital->is_active ? 'Nonaktifkan' : 'Aktifkan' }}">
                                        @if($capital->is_active)
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Pagination -->
        @if($capitalTrackings->hasPages())
            <div class="bg-white px-4 py-3 border-t border-gray-200 sm:px-6">
                {{ $capitalTrackings->links() }}
            </div>
        @endif
    </div>
                ...
                <div class="mt-3">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-lg font-medium text-gray-900">
                            {{ $editingId ? 'Edit Modal Usaha' : 'Tambah Modal Usaha' }}
                        </h3>
                    ...
                    <form wire:submit.prevent="save">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" 
                                   ...
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 @error('initial_amount') border-red-500 @enderror"
                                   placeholder="0"
                                   min="0"
                                   step="0.01">
