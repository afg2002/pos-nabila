...
<div class="space-y-8 animate-fadeInUp">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="mt-4 flex items-center justify-between">
                            <div class="flex items-center space-x-2">
                                ...
                                <span class="text-sm text-green-600 font-semibold">{{ $todaySales->count() }} transaksi</span>
                            </div>
                            <div class="flex items-center text-xs text-gray-500">
                                <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path>
                                </svg>
                                {{ $todaySales->count() > 0 ? '+' . round((($todayRevenue / ($weekRevenue / 7)) - 1) * 100, 1) . '%' : '0%' }}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="relative p-6">
                <div class="flex items-center justify-between">
                    <div class="flex-1">
                        ...
                        <div class="mt-4 flex items-center justify-between">
                            <div class="flex items-center space-x-2">
                                <div class="w-2 h-2 {{ $criticalStock->count() > 0 ? 'bg-red-500' : 'bg-green-500' }} rounded-full {{ $criticalStock->count() > 0 ? 'animate-pulse' : '' }}"></div>
                                <span class="text-sm {{ $criticalStock->count() > 0 ? 'text-red-600' : 'text-green-600' }} font-semibold">
                                    {{ $criticalStock->count() }} kritikal
                                </span>
                            </div>
                            <div class="flex items-center text-xs text-gray-500">
                                <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            <div class="bg-white shadow-xl rounded-2xl border border-gray-200 overflow-hidden">
                ...
                <div class="p-6 space-y-3">
                    ...
                    <a href="{{ route('kasir.management') }}" class="group w-full flex items-center px-4 py-4 text-sm font-medium text-gray-700 bg-gradient-to-r from-green-50 to-green-100 rounded-xl hover:from-green-100 hover:to-green-200 transition-all duration-300 transform hover:scale-105 hover:shadow-lg">
                    ...
                    <a href="{{ route('products.index') }}" class="group w-full flex items-center px-4 py-4 text-sm font-medium text-gray-700 bg-gradient-to-r from-purple-50 to-purple-100 rounded-xl hover:from-purple-100 hover:to-purple-200 transition-all duration-300 transform hover:scale-105 hover:shadow-lg">
            ...
            <div class="bg-white shadow-xl rounded-2xl border border-gray-200 overflow-hidden">
                <div class="bg-gradient-to-r from-orange-50 to-white px-6 py-5 border-b border-gray-200">
                    <div class="flex items-center justify-between">
                        ...
                        <a href="{{ route('incoming-goods-agenda.index') }}" class="text-sm text-orange-600 hover:text-orange-800">Lihat Semua</a>
                    </div>
                </div>
                <div class="p-6 space-y-3">
                    @forelse($upcomingAgendas as $agenda)
                        <div class="p-3 bg-gradient-to-r from-orange-50 to-white rounded-lg border border-orange-200">
                            <div class="flex items-center justify-between">
                                <div class="flex-1">
                                    <p class="text-sm font-semibold text-gray-900">{{ optional($agenda->supplier)->name }}</p>
                                    <p class="text-xs text-gray-500">{{ \Carbon\Carbon::parse($agenda->scheduled_date)->format('d M Y H:i') }}</p>
                                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        <div class="p-4 rounded-xl border border-gray-200 bg-gradient-to-br from-green-50 to-white">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <span class="text-xs font-semibold bg-green-100 text-green-700 px-2 py-1 rounded">{{ $today->isoFormat('D MMM') }}</span>
                            </div>
                            <p class="text-2xl font-bold text-gray-900 mb-1">Rp {{ number_format($todayRevenue, 0, ',', '.') }}</p>
                            <div class="flex items-center justify-between">
                                <span class="text-xs text-gray-500">{{ $todaySales->count() }} transaksi</span>
                                <span class="text-xs text-green-600 font-semibold">{{ $todaySales->count() > 0 ? '+' . round((($todayRevenue / ($weekRevenue / 7)) - 1) * 100, 1) . '%' : '0%' }}</span>
                            </div>
                        </div>
                        ...
                        <div class="p-4 rounded-xl border border-gray-200 bg-gradient-to-br from-blue-50 to-white">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <span class="text-xs font-semibold bg-blue-100 text-blue-700 px-2 py-1 rounded">{{ $weekStart->isoFormat('D MMM') }} - {{ now()->isoFormat('D MMM') }}</span>
                            </div>
                            <p class="text-2xl font-bold text-gray-900 mb-1">Rp {{ number_format($weekRevenue, 0, ',', '.') }}</p>
                            <div class="flex items-center justify-between">
                                <span class="text-xs text-gray-500">{{ $weekSales->count() }} transaksi</span>
                                <span class="text-xs text-blue-600 font-semibold">Rp {{ number_format($weekRevenue / 7, 0, ',', '.') }}/hari</span>
                            </div>
                        </div>
                        ...
                        <div class="p-4 rounded-xl border border-gray-200 bg-gradient-to-br from-purple-50 to-white">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <span class="text-xs font-semibold bg-purple-100 text-purple-700 px-2 py-1 rounded">{{ $monthStart->isoFormat('MMMM') }}</span>
                            </div>
                            <p class="text-2xl font-bold text-gray-900 mb-1">Rp {{ number_format($monthRevenue, 0, ',', '.') }}</p>
                            <div class="flex items-center justify-between">
                                <span class="text-xs text-gray-500">{{ $monthSales->count() }} transaksi</span>
                                <span class="text-xs text-purple-600 font-semibold">Rp {{ number_format($monthRevenue / max(1, now()->diffInDays($monthStart)), 0, ',', '.') }}/hari</span>
                            </div>
                        </div>
                    </div>
                            ...
                            @forelse($recentTransactions as $sale)
                                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                                    ...
                                    <div class="text-right">
                                        <p class="text-sm font-semibold text-gray-900">Rp {{ number_format($sale->final_total, 0, ',', '.') }}</p>
                                        <span class="text-xs px-2 py-1 bg-green-100 text-green-700 rounded-full">{{ $sale->payment_status }}</span>
                                    </div>
                                </div>
                            @empty
                                <div class="text-center py-4">
            ...
            <div class="bg-white shadow-xl rounded-2xl border border-gray-200 overflow-hidden">
                <div class="bg-gradient-to-r from-red-50 to-white px-6 py-5 border-b border-gray-200">
                    <div class="flex items-center justify-between">
                        ...
                        <a href="{{ route('products.index') }}" class="text-sm text-red-600 hover:text-red-800">Kelola Stok</a>
                    </div>
                </div>
                ...
                <div class="p-6">
                    ...
                    @if($criticalStock->count() > 0)
                        <div class="mb-4 p-4 bg-red-50 border border-red-200 rounded-lg">
                            <div class="flex items-center">
                                ...
                                <div>
                                    <p class="text-sm font-semibold text-red-800">{{ $criticalStock->count() }} produk stok kritikal!</p>
                    ...
                    <div class="space-y-2">
                        @forelse($criticalStock as $product)
                            <div class="flex items-center justify-between p-3 bg-gradient-to-r {{ ($product->current_stock ?? 0) <= 0 ? 'from-red-50 to-red-100' : 'from-yellow-50 to-yellow-100' }} border {{ ($product->current_stock ?? 0) <= 0 ? 'border-red-200' : 'border-yellow-200' }} rounded-lg">
                                <div class="flex items-center">
                                    <div class="w-8 h-8 {{ ($product->current_stock ?? 0) <= 0 ? 'bg-red-100' : 'bg-yellow-100' }} rounded-lg flex items-center justify-center mr-3">
                                        <svg class="w-4 h-4 {{ ($product->current_stock ?? 0) <= 0 ? 'text-red-600' : 'text-yellow-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path>
                                        </svg>
                                    </div>
                                    <div>
                                        <p class="text-sm font-medium text-gray-900">{{ $product->name }}</p>
                                        <p class="text-xs text-gray-500">{{ $product->unit_name ?? 'Satuan' }}</p>
                                    </div>
                                </div>
                                <div class="text-right">
                                    <p class="text-lg font-bold {{ ($product->current_stock ?? 0) <= 0 ? 'text-red-600' : 'text-yellow-600' }}">
                                        {{ number_format($product->current_stock ?? 0, 0, ',', '.') }}
                                    </p>
    ...
    <div class="bg-gradient-to-r from-gray-50 to-blue-50 rounded-2xl border border-gray-200 p-4">
        <div class="flex flex-wrap items-center justify-center gap-3">
            <a href="{{ route('kasir.management') }}" class="inline-flex items-center px-4 py-2 text-sm font-semibold text-green-700 bg-green-100 hover:bg-green-200 rounded-md transition-colors">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"></path>
                </svg>
                Kasir (POS)
            </a>
            <a href="{{ route('products.index') }}" class="inline-flex items-center px-4 py-2 text-sm font-semibold text-blue-700 bg-blue-100 hover:bg-blue-200 rounded-md transition-colors">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path>
                </svg>
                Kelola Produk
            </a>
            <a href="{{ route('products.import') }}" class="inline-flex items-center px-4 py-2 text-sm font-semibold text-purple-700 bg-purple-100 hover:bg-purple-200 rounded-md transition-colors">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v16h16M4 4l8 8"></path>
                </svg>
                Import Stok
            </a>
            <a href="{{ route('suppliers.index') }}" class="inline-flex items-center px-4 py-2 text-sm font-semibold text-orange-700 bg-orange-100 hover:bg-orange-200 rounded-md transition-colors">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path>
                </svg>
                Supplier
            </a>
            <a href="{{ route('incoming-goods-agenda.index') }}" class="inline-flex items-center px-4 py-2 text-sm font-semibold text-red-700 bg-red-100 hover:bg-red-200 rounded-md transition-colors">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                </svg>
                Jadwal Datang
